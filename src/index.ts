...
import { CustomPaymasterHandler, CustomPaymasterParameters } from './types';
...
const GENERAL_FLOW_PAYMASTER = `${ZYFI_ENDPOINT}sponsored_paymaster/v1`;
...
export interface ZyfiPaymasterParams {
  ...
  replayLimit?: number;
  /** turn it on when using session keys */
  generalFlow?: boolean;
  /** for testing purposes */
  apiUrl?: string;
}

export function createZyfiPaymaster(
  params: ZyfiPaymasterParams
): CustomPaymasterHandler {
  if (!params.apiKey) throw new Error('ZyFi: Provide API KEY');
  if (
    params.sponsorshipRatio !== undefined &&
    (params.sponsorshipRatio > 100 || params.sponsorshipRatio < 0)
  )
